...
export default function StatsPanel({ metrics, model, onLaunch, launching, launchFee, fluxBalance, launchError }: StatsPanelProps) {
  const modelDef = ROCKET_MODELS.find((entry) => entry.id === model) ?? ROCKET_MODELS[0];
  const canAfford = fluxBalance >= launchFee;
    ...
    <div className="border overflow-hidden" style={{ background: 'var(--color-bg-base)', border: '1px solid var(--color-border-subtle)' }}>
      ...
      <div className="p-5">
        ...
        <div
          ...
          style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)' }}
        >
          <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-text-muted">Launch Fee</p>
          <div className="mt-1 flex items-center justify-between">
            <span className="font-mono font-bold text-sm text-text-primary">
              {launchFee.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} FLUX
            </span>
            <span className="font-mono text-xs text-text-muted">
              Balance: {fluxBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
          </div>
        </div>
        ...
        {launchError && (
          ...
          >
            <p className="text-xs font-mono text-red-400">{launchError}</p>
          </div>
        )}
        ...
        <button
          onClick={onLaunch}
          disabled={!metrics.canLaunch || launching || !canAfford}
          ...
          style={metrics.canLaunch && !launching && canAfford ? {
        ...
        >
          ...
          ) : !canAfford && metrics.canLaunch ? (
            <>
              <Rocket size={18} style={{ color: 'var(--color-text-muted)' }} />
              Insufficient FLUX
            </>
          ) : (
            <>
