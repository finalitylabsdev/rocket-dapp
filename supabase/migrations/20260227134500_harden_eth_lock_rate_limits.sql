/*
  # Harden ETH lock submission rate limits

  ## Summary
  - Adds a per-user throttle to `record_eth_lock_sent(...)`
  - Limits repeated ETH lock submissions to 6 attempts per 15 minutes
*/
...
DECLARE
  ...
  v_recent_eth_lock_events integer;
BEGIN
  v_user_id := auth.uid();
  ...
  SELECT count(*) INTO v_recent_eth_lock_events
  FROM app_logs
  WHERE auth_user_id = v_user_id
    AND event_name = 'eth_lock_sent'
    AND created_at >= now() - interval '15 minutes';

  IF v_recent_eth_lock_events >= 6 THEN
    RAISE EXCEPTION 'rate limit exceeded for eth lock submissions';
  END IF;
